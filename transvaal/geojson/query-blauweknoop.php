...
SELECT ?aladr (MAX(?lopo) as ?lp) (MAX(?wktspatial) as ?wkt) (GROUP_CONCAT(DISTINCT ?adrstr;SEPARATOR=",") as ?labels) (count(DISTINCT ?po) as ?residents) WHERE {
  ...
  ?aladr schema:geoContains ?lopo .
  ?lopo geo:asWKT ?wktspatial .
  ?deed rico:hasOrHadSubject ?p .
  ?p saa:relatedPersonObservation ?po .
  ';
